...
test.describe('Civic Auth Applications', () => {
  ...
  test('should complete email verification flow with actual email', async ({ page, browserName }) => {
    ...
    const currentLoginFlowId = extractedLoginFlowId;
    ...
    await db.connect();
    const verificationCode = await db.waitForEmailCode(currentLoginFlowId, 15, 2000);
    await db.disconnect();

    console.log(`Retrieved verification code: ${verificationCode} for loginFlowId: ${currentLoginFlowId}`);
