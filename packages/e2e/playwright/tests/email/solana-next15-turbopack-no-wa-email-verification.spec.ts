...
test.describe('Solana Next.js 15 Turbopack No Wallet Adapter Email Verification Tests', () => {
  test.beforeEach(async ({ page }) => {
    await allure.epic('Sample Applications');
    await allure.feature('Solana Next.js 15 Turbopack No Wallet Adapter Email Verification');
  });

  test('should complete email verification flow without wallet adapter', async ({ page, browserName }) => {
    await allure.story('Solana Next.js 15 Turbopack No Wallet Adapter Email Code Verification Flow');
    await allure.severity('critical');
    await allure.tag('solana-nextjs15-turbopack-no-wa-email-verification');
